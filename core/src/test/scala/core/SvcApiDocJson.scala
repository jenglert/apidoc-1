...
class SvcApiDocJson extends FunSpec with Matchers {
  ...
  it("parses models") {
    ...
    service.models.map(_.name).sorted.mkString(" ") should be("code code_error error membership membership_request membership_request_form organization organization_form service user user_form version")
